...
// then throws at runtime inside the wasm (Host.getFunctions()[name] is
// undefined) — which the state builder swallows as a graceful degrade, so
// the breakage is silent (v0.4.0 shipped with session birds dead this way).
// Guard the declaration list against the calls actually made in src/.
